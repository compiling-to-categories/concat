...
-- Has a corresponding value representation
class HasV a where
  type V a
  type V a = a -- default
  toV :: a -> V a
  unV :: V a -> a
  -- Must be isomorphism. Maybe wrap as such.
  default toV :: V a ~ a => a -> V a
  toV = id
  default unV :: V a ~ a => V a -> a
  unV = id
...
instance HasV Int
instance HasV Float
instance HasV Double

instance (HasV a, HasV b) => HasV (a :* b) where
  type V (a :* b) = V a :* V b
  toV = toV *** toV
  unV = unV *** unV
...
instance OpCon (:*) (Sat HasV) where
  inOp = Entail (Con.Sub Con.Dict)
  {-# INLINE inOp #-}
...
data Closure a b = forall e. HasV e => Closure (StackFun (e :* a) b) (V e)

instance (HasV a, HasV b) => HasV (a -> b) where
  type V (a -> b) = Closure a b
  toV f = Closure (stackFun (f . lcounit)) ()
  unV (Closure ve sf) = unstackFun sf . (unV ve ,)

{--------------------------------------------------------------------
    Stack functions (for specification)
--------------------------------------------------------------------}

type First a b = forall z. a :* z -> b :* z

unfirst :: First a b -> (a -> b)
unfirst h = rcounit . h . runit

type FirstV a b = First (V a) (V b)

unfirstV :: (HasV a, HasV b) => FirstV a b -> (a -> b)
unfirstV h = unV . unfirst h . toV

-- | Category of stack functions
newtype StackFun a b = SF (FirstV a b)

inSF :: (FirstV a b -> FirstV a' b') -> (StackFun a b -> StackFun a' b')
inSF w (SF h) = SF (w h)

inSF2 :: (FirstV a b -> FirstV a' b' -> FirstV a'' b'')
      -> (StackFun a b -> StackFun a' b' -> StackFun a'' b'')
inSF2 w (SF h) (SF k) = SF (w h k)

-- | The semantic functor that specifies 'StackFun'.
stackFun :: (HasV a, HasV b) => (a -> b) -> StackFun a b
stackFun f = SF (first (toV . f . unV))
{-# INLINE stackFun #-}

unstackFun :: (HasV a, HasV b) => StackFun a b -> (a -> b)
unstackFun (SF h) = unfirstV h

instance Category StackFun where
  type Ok StackFun = HasV
  id = stackFun id
  -- SF g . SF f = SF (g . f)
  (.) = inSF2 (.)
  {-# INLINE id #-}
...
evalPrim (f :+++ g) = unstackOps f +++ unstackOps g
...
unstackOp :: StackOp u v -> (u -> v)
unstackOp (Pure f) = first (evalPrim f)
unstackOp Push     = rassocP
unstackOp Pop      = lassocP
...
unstackOps :: StackOps a b -> (a -> b)
unstackOps Nil          = id
unstackOps (op :< rest) = unstackOps rest . unstackOp op
...
-- TODO: Replace use of Show2 with a quantified construct.
...
progFun (SP ops) = SF (unstackOps ops)

evalProg :: StackProg a b -> (a -> b)
evalProg = unstackFun . progFun

-- evalProg (SP ops) = rcounit . unstackOps ops . runit
-- evalProg (SP ops) a = fst (unstackOps ops (a,()))
