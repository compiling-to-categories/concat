...
basicTests = testGroup "basic tests"
  ...
  -- TODO(sandy): new name
  -- , runSyn $ toCcc $ andDerR $ fmap @(Vector 5) @R cos {-Circ "fmap-cos-adr"-}
  ...
  -- , runCirc "unV-r-v2" $ toCcc $ unV @R @(Vector 2 R)
  ...
  -- , runCirc "distribute-pair-pair-r" $ toCcc (distribute @Pair @Pair @R)
  ...
  -- , runCirc "y-plus-x" $ toCcc $ \(x, y) -> y + x :: R

  -- , runCirc "const-y" $ toCcc $ \(_x :: R, y :: R) -> y
  ...
  -- , runSyn $ toCcc $ uncurry ((==) @(Bool :* Int)) {-Circ "equal-pair-b"-}
